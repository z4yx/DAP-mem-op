...
class SvfFormatter:
    """Formats JTAG operations as SVF (Serial Vector Format) syntax.
    ...
    To support alternative output formats (e.g. WGL), create a class with
    the same public interface and pass it to ``SvfGenerator`` via the
    ``formatter`` parameter.
    """
    ...
    def emit(self, line: str):
        """Write a raw line to the output stream."""
        self._f.write(f"{line}\n")

    def emit_header(self, chain_desc: str):
        """Emit the SVF preamble."""
        self._f.write(
            textwrap.dedent(f"""\
            // ============================================================================
    ...
    def sir(self, bits: int, tdi: int):
        """Emit an SIR (Scan Instruction Register) SVF statement.

        Args:
            bits: Total bit width of the scan (includes bypass TAPs).
            tdi:  TDI value (includes bypass padding).
        """
        self.emit(
    ...
    def sdr(
        ...
        """Emit an SDR (Scan Data Register) SVF statement.

        Args:
            bits:    Total bit width of the scan (includes bypass TAPs).
            tdi:     TDI value (includes bypass padding).
            comment: Optional human-readable annotation.
            tdo:     Expected TDO value for verification (full chain width).
            mask:    TDO comparison mask (1 = check, 0 = ignore).
        """
        parts = (
            f"SDR {bits} TDI ({self._hex(tdi, bits)}) "
            f"SMASK ({self._mask(bits)})"
        )
        if tdo is not None and mask is not None:
            parts += (
                f" TDO ({self._hex(tdo, bits)}) "
                f"MASK ({self._hex(mask, bits)})"
            )
        tail = f"  // {comment}" if comment else ""
        self.emit(f"{parts};{tail}")

    def runtest(self, tck_cycles: int):
        """Emit a RUNTEST SVF statement."""
        if tck_cycles > 0:
            self.emit(f"RUNTEST {tck_cycles} TCK;")

    def state_reset_idle(self):
        """Emit STATE RESET IDLE."""
        self.emit("STATE RESET IDLE;")

    def comment(self, text: str):
        """Emit a comment line (SVF ``// ...`` syntax)."""
    ...
    def _mask(bits: int) -> str:
        ...
        return SvfFormatter._hex((1 << bits) - 1, bits)
...
class SvfGenerator:
    ...
    Combines JTAG sequencing logic (chain composition, DAP protocol) with
    the ``SvfFormatter`` output layer.  To emit a different output format,
    ...
    def __init__(
        ...
        Args:
            output_file: Path to SVF output file (None = stdout).  Ignored
                         when *formatter* is provided.
    ...
    def comment(self, text: str):
        """Emit a human-readable comment line (delegates to formatter)."""
        self._formatter.comment(text)

    def blank(self):
        """Emit a blank line for readability (delegates to formatter)."""
        self._formatter.blank()
    ...
    def close(self):
        """Flush and close the output stream (delegates to formatter)."""
...
class ArmCpuSvfBuilder:
    """Orchestrates the full SVF sequence for ARM CPU memory download.

    Supports both ADIv5 (e.g. Cortex-A9, Cortex-M4) and ADIv6
    ...
    ARM_CPU_BASE_CID = 0x6BA02477  # Example: Cortex-R52 r1p0
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Generate SVF JTAG sequences for ARM CPU memory access "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(DBGSWENABLE, ADIv6 only).  Default: 5.  "
